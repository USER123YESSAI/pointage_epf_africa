...
public class SeanceDAO extends AbstractDAO<SeancePlanifiee, Long> {
        ...
        try (Session session = openSession()) {
            ...
            return session.createQuery(
                            "SELECT DISTINCT s FROM SeancePlanifiee s " +
                                    "JOIN FETCH s.assignation a JOIN FETCH a.cours " +
                                    "LEFT JOIN FETCH a.salle LEFT JOIN FETCH a.professeur " +
                                    "WHERE s.dateHeure >= :debut AND s.dateHeure < :fin ORDER BY s.dateHeure",
        ...
        try (Session session = openSession()) {
            ...
            return session.createQuery(
                            "SELECT DISTINCT s FROM SeancePlanifiee s " +
                                    ...
                                    "AND s.dateHeure >= :debut AND s.dateHeure < :fin ORDER BY s.dateHeure",
                            SeancePlanifiee.class)
        ...
        try (Session session = openSession()) {
            ...
            return session.createQuery(
                            "SELECT COUNT(s) FROM SeancePlanifiee s " +
                                    "WHERE s.statut = :statut AND s.dateHeure >= :debut AND s.dateHeure < :fin",
                            Long.class)
                    .setParameter("statut", StatutSeance.PLANIFIEE)
                    .setParameter("debut", debut).setParameter("fin", fin).uniqueResult();
        }
    }

    public long countSeancesPlanifieesParMoisEtProfesseur(int mois, int annee, Long professeurId) {
        try (Session session = openSession()) {
            LocalDateTime debut = LocalDateTime.of(annee, mois, 1, 0, 0);
            LocalDateTime fin   = debut.plusMonths(1);
            return session.createQuery(
                            "SELECT COUNT(s) FROM SeancePlanifiee s " +
                                    "JOIN s.assignation a " +
                                    "WHERE a.professeur.id = :profId " +
                                    "AND s.statut = :statut " +
    ...
    public List<SeancePlanifiee> findByPeriodeEtProfesseur(LocalDateTime debut,
                                                           LocalDateTime fin,
                                                           Long professeurId) {
        try (Session session = openSession()) {
            return session.createQuery(
                            "SELECT DISTINCT s FROM SeancePlanifiee s " +
                                    ...
                                    "LEFT JOIN FETCH a.salle LEFT JOIN FETCH a.professeur " +
                                    "WHERE s.dateHeure >= :debut AND s.dateHeure <= :fin " +
                                    "AND a.professeur.id = :profId ORDER BY s.dateHeure",
                            SeancePlanifiee.class)
                    .setParameter("debut", debut).setParameter("fin", fin)
